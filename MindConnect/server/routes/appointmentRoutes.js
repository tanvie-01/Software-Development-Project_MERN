...
// ২. সব অ্যাপয়েন্টমেন্ট (Admin) - 🔥 Updated: Newest First
router.get("/", async (req, res) => {
  try {
    // createdAt: -1 মানে লেটেস্ট আগে দেখাবে
...
// ৩. ইউজারের অ্যাপয়েন্টমেন্ট (User) - 🔥 Updated: Newest First
...
// ৫. ডাক্তারের অ্যাপয়েন্টমেন্ট (Doctor) - 🔥 Updated: Newest First
router.get("/doctor/:doctorName", async (req, res) => {
  try {
    const appointments = await Appointment.find({ doctorName: req.params.doctorName }).sort({ createdAt: -1 });
    res.json(appointments);
  } catch (error) {
    res.status(500).json({ message: "Server Error" });
  }
