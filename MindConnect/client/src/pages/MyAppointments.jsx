...
const MyAppointments = () => {
  ...
  useEffect(() => {
    ...
    if (userInfo) {
      ...
      fetchAppointments(JSON.parse(userInfo)._id);
    } else navigate("/login");
  }, [navigate]);
  ...
  const fetchAppointments = async (userId) => {
    try {
      const { data } = await axios.get(
        `https://mindconnect-backend-afyf.onrender.com/api/appointments/${userId}`
      );
  ...
  const openPrescription = (text) => {
    setSelectedPrescription(text);
  ...
  const submitReview = async () => {
    try {
      await axios.post(
        `https://mindconnect-backend-afyf.onrender.com/api/doctors/${reviewDoctorId}/reviews`,
        { rating, comment, userName: user.name, userId: user._id }
      );
      alert("Review Added!");
                  ...
                  >
                    ...
                    <td className="px-6 py-5 flex gap-3">
                      {appt.status === "completed" && appt.prescription && (
                        <button
                          onClick={() => openPrescription(appt.prescription)}
                          className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold shadow hover:bg-blue-700 transition"
                        >
                      ...
                      {appt.status === "completed" && (
                        <button
                          onClick={() => openReviewModal(appt.doctorId)}
                          className="flex items-center gap-2 bg-yellow-400 text-white px-4 py-2 rounded-lg text-sm font-bold shadow hover:bg-yellow-500 transition"
                        >
                          <span>⭐</span> Rate
                        </button>
                      )}
          ...
          <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-lg relative animate-bounce-in">
            ...
            <div className="mt-8 text-center">
              <button
                ...
                className="bg-gray-800 text-white px-6 py-2.5 rounded-lg font-bold shadow hover:bg-gray-900 transition mr-4"
              >
                Print / PDF
              </button>
              <button
                onClick={() => setShowPrescriptionModal(false)}
                className="bg-blue-600 text-white px-6 py-2.5 rounded-lg font-bold shadow hover:bg-blue-700 transition"
              >
                Close
              </button>
            </div>
          ...
          <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md animate-bounce-in">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  className="w-full border p-3 rounded-xl focus:ring-2 ring-yellow-400 outline-none"
                  value={rating}
              ...
              <div>
                ...
                <textarea
                  className="w-full border p-3 rounded-xl h-28 focus:ring-2 ring-yellow-400 outline-none"
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  ...
                  className="bg-gray-200 text-gray-700 px-5 py-2.5 rounded-xl font-bold hover:bg-gray-300"
                >
                  Cancel
                </button>
                <button
                  onClick={submitReview}
                  className="bg-yellow-500 text-white px-5 py-2.5 rounded-xl font-bold shadow hover:bg-yellow-600"
                >
                  Submit
                </button>
              </div>
