...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "https://mindconnect-backend-afyf.onrender.com/api/users/forgot-password",
        { email }
      );
  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          "url('https://images.unsplash.com/photo-1518531933037-91b2f5f229cc?q=80&w=1527&auto=format&fit=crop')",
      }}
    >
      ...
      <div className="relative z-10 w-full max-w-md p-8 bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl animate-fade-in-up">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            className="w-full py-3.5 rounded-xl bg-gradient-to-r from-yellow-500 to-orange-600 text-white font-bold text-lg shadow-lg hover:shadow-yellow-500/40 hover:scale-[1.02] transition duration-300 active:scale-95"
          >
            Send Reset Link
          </button>
        </form>
        ...
        <div className="mt-8 text-center">
          <Link
            to="/login"
            className="flex items-center justify-center gap-2 text-sm text-white/80 hover:text-white transition font-medium"
          >
